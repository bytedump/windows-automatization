...
# Stop on non-terminating errors too, so per-step try/catch actually catches them. The tolerate-
# and-continue calls already carry explicit -ErrorAction SilentlyContinue. Matches setup.ps1.
...
function Clear-AutoLogon {
    ...
    # Trust the registry state, not the (suppressed) cmdlet return. A $null read is a READ FAILURE,
    # not proof the password is gone (audit A12) - fail closed so it is never mistaken for "verified".
    $rp = Get-ItemProperty -Path $winlogon -ErrorAction SilentlyContinue
    if ($null -eq $rp) {
        Write-CleanupLog ERROR 'AutoLogon read-back FAILED (could not read the Winlogon key) - cannot verify the password was removed.'
        Write-CriticalMarker -Reason 'Clear-AutoLogon read-back could not read HKLM Winlogon - plaintext password state unknown'
        return $false
    }
    $pwGone  = -not $rp.PSObject.Properties['DefaultPassword']
    $autoOff = (-not $rp.PSObject.Properties['AutoAdminLogon']) -or ($rp.AutoAdminLogon -eq '0')
...
function Invoke-Cleanup {
    ...
    # Wait for Phase B to finish (monotonic clock). ALWAYS preserve the Phase B log afterward.
    ...
    # Teardown (unregister the tasks + delete the staging) is gated on Phase B ACTUALLY completing
    # ($done), NOT merely on AutoLogon being cleared (audit C2). Otherwise, when the new user's
    # AutoLogon never fires (e.g. Credential Guard on 24H2/25H2) and a technician logs in as admin,
    # this SYSTEM task would clear AutoLogon, time out waiting for a user-done that can only come
    # from the NEW user's session, then delete phase-b.ps1 / state.json / signatures / VPN - leaving
    # Phase B permanently unrecoverable. On timeout, leave everything intact so Phase B still runs on
    # the new user's first real logon; a later cleanup run tears down once user-done exists.
    if (-not $done) {
        ...
        Write-CleanupLog INFO 'Leaving Phase B task + staging intact so it can still run on the new user logon; no teardown. AutoLogon was already cleared.'
        return
    }
    Write-CleanupLog OK 'Phase B signaled completion (user-done flag seen)'

    # Phase B is done: the per-user task has served its purpose and can be removed either way.
    Unregister-PhaseTasks -TaskNames @($script:UserTaskName)
    if (-not $cleared) {
        # Security not verified closed: keep the SYSTEM task + staging so the next logon retries
        # Clear-AutoLogon (idempotent). Do NOT self-destruct.
        Write-CleanupLog ERROR 'AutoLogon not verified cleared - leaving SYSTEM task + staging for retry at next logon.'
        return
    }

    # Phase B done AND AutoLogon verified cleared: full teardown.
