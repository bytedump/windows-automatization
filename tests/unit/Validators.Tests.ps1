...
Describe 'Test-Username' {
    ...
    It 'accepts a name at the 20-char SAM limit' { Test-Username 'wellington.nascimeno' | Should -BeTrue }  # exactly 20 chars
    ...
    It 'rejects invalid username <u>' -ForEach @(
        ...
        @{ u = '' }
        @{ u = 'wellington.nascimento' }    # 21 chars - exceeds the SAM limit (audit A3)
...
Describe 'Format-Username' {
    ...
    It 'builds the login from a full name typed in one box' {
        Format-Username 'Joao Silva' '' | Should -BeExactly 'joao.silva'
    }
    It 'handles a one-box name with accents' {
        Format-Username 'João Silva' '' | Should -BeExactly 'joao.silva'
    }
}
