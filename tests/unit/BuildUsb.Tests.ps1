...
Describe 'build-usb.ps1 password validation' {
    ...
    It 'rejects a password shorter than the minimum' {
        { & $BuildUsb -TemplatePath $tpl -OutPath $out -AdminUser setupadmin -AdminPassword (New-Pw 'short123') } |
            Should -Throw '*too short*'
        Test-Path $out | Should -BeFalse
    }

    It 'accepts a password at the minimum length (12) and writes the file' {
